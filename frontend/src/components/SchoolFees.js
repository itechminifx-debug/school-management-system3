...
function SchoolFees() {
  ...
  const fetchFeeSummary = async () => {
    ...
    try {
      const response = await axios.get(
        `${apiUrl}/api/school-fees/summary/${selectedClass}/${selectedTerm}/${academicYear}?t=${Date.now()}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const getStatusBadge = (status) => {
    if (status === 'paid') return <span style={{ background: '#10b981', color: 'white', padding: '4px 8px', borderRadius: '20px', fontSize: '11px', fontWeight: 'bold' }}>✅ PAID</span>;
    if (status === 'partial') return <span style={{ background: '#f59e0b', color: 'white', padding: '4px 8px', borderRadius: '20px', fontSize: '11px', fontWeight: 'bold' }}>⚠️ PARTIAL</span>;
    return <span style={{ background: '#ef4444', color: 'white', padding: '4px 8px', borderRadius: '20px', fontSize: '11px', fontWeight: 'bold' }}>❌ UNPAID</span>;
  };
      ...
      <div className="card">
        ...
        {feeData && (
          <div className="stats-grid" style={{ marginBottom: '1.5rem' }}>
            ...
            <div className="stat-card"><h3 style={{ color: '#10b981' }}>{feeData.paid_count}</h3><p>Fully Paid ✅</p></div>
            <div className="stat-card"><h3 style={{ color: '#f59e0b' }}>{feeData.partial_count}</h3><p>Partial ⚠️</p></div>
            <div className="stat-card"><h3 style={{ color: '#ef4444' }}>{feeData.unpaid_count}</h3><p>Not Paid ❌</p></div>
              ...
              <tbody>
                {feeData.students.map(student => (
                  <tr key={student.id} style={{ borderBottom: '1px solid #ddd', background: student.status === 'partial' ? '#fff3cd' : student.status === 'unpaid' ? '#f8d7da' : 'white' }}>
                    ...
                    <td style={{ color: '#10b981' }}>₵{student.amount_paid}</td>
          ...
          <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
            <div style={{ background: 'white', padding: '2rem', borderRadius: '12px', width: '700px', maxHeight: '80%', overflowY: 'auto' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '1rem' }}>
                <button onClick={() => { setShowHistoryModal(false); setStudentHistory(null); }} style={{ background: '#95a5a6', padding: '8px 16px', border: 'none', borderRadius: '4px', cursor: 'pointer', color: 'white' }}>Close</button>
              </div>
            </div>
          </div>
        )}
          ...
          <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
            <div style={{ background: 'white', padding: '2rem', borderRadius: '12px', width: '400px' }}>
              ...
              <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', marginTop: '1rem' }}>
                ...
                <button onClick={() => { setShowReceiptModal(false); setReceiptData(null); }} style={{ background: '#95a5a6', padding: '8px 16px', border: 'none', borderRadius: '4px', cursor: 'pointer', color: 'white' }}>Close</button>
              </div>
            </div>
          </div>
        )}
